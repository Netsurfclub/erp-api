...
class CreateSupplierInputTestObject {
    companion object {
        ...
        fun input1WithLongName(): CreateSupplierInput = build(name = SUPPLIER_LONG_NAME)

        fun input1WithShortPhone(): CreateSupplierInput = build(phone = "1")

        fun input1WithLongPhone(): CreateSupplierInput = build(phone = SUPPLIER_LONG_PHONE)

        fun input1WithNullPhone(): CreateSupplierInput = build(phone = null)

        fun input1WithShortEmail(): CreateSupplierInput = build(email = "a")

        fun input1WithLongEmail(): CreateSupplierInput = build(email = SUPPLIER_LONG_EMAIL)

        fun input1WithInvalidEmail(): CreateSupplierInput = build(email = SUPPLIER_INVALID_EMAIL)
